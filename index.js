...
async function LogCommandExecution(client, source, commandName, commandType, optionsString) {
    ...
    try {
        // Fetch the channel in case it's not cached
...
client.on(Events.ClientReady, () => {
    ...
    client.user.setActivity('Merry Christmas', { type: ActivityType.Custom });
...
async function InteractionHandler(interaction, type) {
    ...
    if (!component) return;
    try {
        // Autocomplete interactions should call the module's `autocomplete` handler
        if (interaction.isAutocomplete() && typeof component.autocomplete === 'function') {
            return await component.autocomplete(interaction, client);
        }

        // Modal submits and buttons/commands use execute
...
client.on('interactionCreate', async (interaction) => {
    if (interaction.isAutocomplete()) return InteractionHandler(interaction, 'commands');
